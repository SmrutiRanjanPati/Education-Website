/* =============================================
   ...
   With spam-prevention Math Captcha Validation
   ============================================= */
...
(function () {
  ...
  function validateFormInputs(formId, fields, expectedCaptcha) {
    ...
    if (!fields.fullName || fields.fullName.length < 6) {
      const idSelector =
        formId === "enquiry-popup-form" ? "#enq-fullname" : "#cp-fullname";
      return triggerError(
        idSelector,
        "⚠️ Full Name must be at least 6 characters long.",
      );
    }
    ...
    if (!mobileRegEx.test(fields.mobile) || /^(\d)\1{9}$/.test(fields.mobile)) {
      const idSelector =
        formId === "enquiry-popup-form" ? "#enq-mobile" : "#cp-mobile";
      return triggerError(
        idSelector,
    ...
    if (!emailRegEx.test(fields.email)) {
      const idSelector =
        formId === "enquiry-popup-form" ? "#enq-email" : "#cp-email";
      return triggerError(
        idSelector,
        "⚠️ Please provide a valid email address structure.",
      );
    }

    if (!fields.details || fields.details.length < 20) {
      const idSelector =
        formId === "enquiry-popup-form" ? "#enq-message" : "#cp-message";
      return triggerError(
        idSelector,
        "⚠️ Details must be at least 20 characters.",
      );
    }

    if (parseInt(fields.captchaUserAnswer) !== expectedCaptcha) {
      const idSelector =
        formId === "enquiry-popup-form"
          ? "#enq-captcha-input"
          : "#cp-captcha-input";
  ...
  function openModal(collegeName, courseName) {
    ...
    // The text MUST be wrapped in a <span> for the ellipsis engine to trigger safely
    ...
    setTimeout(() => {
      document.getElementById("enq-fullname").focus();
    }, 350);
  }
  function closeModal() {
  ...
  function attachModalEvents(overlay) {
    ...
    document
      .getElementById("enquiry-popup-form")
      .addEventListener("submit", handleEnquirySubmit);

    document
      .getElementById("enq-mobile")
    ...
    document
      .getElementById("enq-captcha-input")
      .addEventListener("input", function () {
        this.value = this.value.replace(/[^0-9]/g, "");
        this.style.borderColor = "";
      });
    ...
    document
      .querySelectorAll(
        "#enquiry-popup-form input, #enquiry-popup-form textarea",
      )
      .forEach((el) => {
  ...
  async function handleEnquirySubmit(e) {
    ...
    try {
      ...
      errorMsg.textContent =
        "❌ Submission failed. Verify parameters and retry.";
      errorMsg.classList.add("show");
    }
  }
  ...
  function openContactModal() {
    ...
    document.body.style.overflow = "hidden";
    setTimeout(() => {
      document.getElementById("cp-fullname").focus();
    }, 350);
  }
  ...
  function resetContactModal() {
    ...
    document.getElementById("contact-popup-form-section").style.display =
      "block";
  ...
  function attachContactModalEvents(overlay) {
    ...
    document
      .getElementById("cp-captcha-input")
      .addEventListener("input", function () {
        this.value = this.value.replace(/[^0-9]/g, "");
        this.style.borderColor = "";
      });
  ...
  async function handleContactSubmit(e) {
    ...
    try {
      ...
      errorMsg.textContent =
        "❌ Transmission failed. Verify parameters and retry.";
      errorMsg.classList.add("show");
    }
  }
  ...
  function showThankYou(formSectionId, thankyouId, countdownId, closeFn) {
    ...
    const interval = setInterval(() => {
      ...
      countdownEl.textContent =
        seconds > 0
          ? `Closing window in ${seconds}s...`
          : "Resetting layout viewport...";
      if (seconds <= 0) {
    ...
    document.body.addEventListener("click", function (e) {
      ...
      if (enquiryTrigger) {
        ...
        const collegeName =
          ...
          card
            ?.querySelector(
              ".news-content h6, .news-content h5, .news-content h4",
            )
            ?.textContent.trim() ||
